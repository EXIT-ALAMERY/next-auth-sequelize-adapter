  ...
  async function getAdapter (appOptions) {
    ...
    async function getUserByProviderAccountId (providerId, providerAccountId) {
      ...
      try {
        ...
        return models.User.findOne({ where: { id: account.userId } })
      } catch (error) {
    ...
    async function getSession (sessionToken) {
      ...
      try {
        const session = await models.Session.findOne({ where: { sessionToken } })
    ...
    async function updateSession (session, force) {
      ...
      try {
        ...
        return models.Session.update({ expires: expires.toISOString() }, {
          where: { id }
        })
      } catch (error) {
    ...
    async function deleteSession (sessionToken) {
      ...
      try {
        return models.Session.destroy({ where: { sessionToken } })
      } catch (error) {
    ...
    async function getVerificationRequest (identifier, token, secret, provider) {
      ...
      try {
        ...
        const hashedToken = createHash('sha256').update(`${token}${secret}`).digest('hex')
        const verificationRequest = await models.VerificationRequest.findOne({ where: { token: hashedToken } })

        if (verificationRequest && verificationRequest.expires && new Date() > verificationRequest.expires) {
          // Delete verification entry so it cannot be used again
          await models.VerificationRequest.destroy({ where: { token: hashedToken } })
          return null
        }
